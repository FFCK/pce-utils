...
const prefix = 'r4_';
const folder = 'FranceElite2019';
const suffix =  '_FB.txt';

const cats = ['K1H', 'K1D', 'C1H', 'C1D'];

const filterOnCountry = 'FRA';
...
pceParser.parseEmbarcations((licences) => {
  ...
  function iterator(cat, iteratorCallback) {
    const columns = ["rank", "bib", "name", "country", "club", "time", "penalties","total"];
    ...
    csv2Array({
      ...
      for(let line of siwi) {
        ...
        if(line.rank !== 'Touched' && line.rank !== 'Missed') {
          if(filterOnCountry !== '' && line.country === filterOnCountry) {
            //console.log(line);
            ...
            if(cat !== 'C2H' && cat !== 'C2M' && cat !== 'C2D') {
              ...
              //console.log(found);
              if(!found) {
                console.log(`${firstname} ${lastname} (${cat}) not found.`);
              } else {
